...
public class SRM640_Div2_L2_NumberGameAgain
{
	public long solve(int k, long[] table)
	{
		...
		for (int i = 0; i < n; i++)
			...
			{
				long x = table[j];
				while (x > table[i] && bad[j] == 0)
				{
					x /= 2;
					if (x == table[i]) bad[j] = 1;
				}
			}
		...
		for (int i = 0; i < n; i++)
		{
			...
			int j = 1;
			for (j = 1; j <= k; j++)
				if (pow2[j] > table[i]) break;
			
			tot -= (pow2[k-j+1] - 1);
			//tot -= Math.pow(2,  k - (int)(Math.log(table[i])/Math.log(2)))-1;
		}
		return tot;
	...
	private static boolean KawigiEdit_RunTest(int testNum, int p0, long[] p1, boolean hasAnswer, long p2) {
		...
		System.out.print("}");
		System.out.println("]");
		SRM640_Div2_L2_NumberGameAgain obj;
		long answer;
		obj = new SRM640_Div2_L2_NumberGameAgain();
