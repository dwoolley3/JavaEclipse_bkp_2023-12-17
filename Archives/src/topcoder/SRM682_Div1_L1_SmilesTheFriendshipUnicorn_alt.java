...
public class SRM682_Div1_L1_SmilesTheFriendshipUnicorn_alt
{	
	ArrayList<Integer>[] g;
	boolean[] used;
		...
		for (int x = 0; x < N; x++)
			for (int y : g[x])
				for (int z : g[y]) if (z != x) {
					for (int w : g[z]) if (w != x && w != y) {
						if (g[w].size() < 5) {
							for (int u : g[w]) if (u != x && u != y && u != z) 
								return "Yay!";
						}
						else return "Yay!";
					}
	...
	private static boolean KawigiEdit_RunTest(int testNum, int p0, int[] p1, int[] p2, boolean hasAnswer, String p3) {
		...
		System.out.print("}");
		System.out.println("]");
		SRM682_Div1_L1_SmilesTheFriendshipUnicorn_alt obj;
		String answer;
		obj = new SRM682_Div1_L1_SmilesTheFriendshipUnicorn_alt();
