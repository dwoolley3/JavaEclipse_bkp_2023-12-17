...
public class Program_fr_pw2_GCJ
{	
	FastReader in;
	PrintWriter out;
	
	public static void main(String[] args)  {
		new Program_fr_pw2_GCJ().runWithFiles();
	}
	...
	void runWithFiles() {
		...
		//in = new FastReader(new File("src/A-small.in"));
		//in = new FastReader(new File("src/A-large.in"));
		try {
			out = new PrintWriter(new File("src/A-ex.out"));
			//out = new PrintWriter(new File("src/A-small.out"));
