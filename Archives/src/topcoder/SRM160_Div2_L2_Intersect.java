...
public class SRM160_Div2_L2_Intersect
{
	public int area(int[] x, int[] y)
	{
        ...
        {
            x1 = Math.max(x1, Math.min(x[i], x[i+1]));
            x2 = Math.min(x2, Math.max(x[i], x[i+1]));
            y1 = Math.max(y1, Math.min(y[i], y[i+1]));
            y2 = Math.min(y2, Math.max(y[i], y[i+1]));           
        }
	...
	private static boolean KawigiEdit_RunTest(int testNum, int[] p0, int[] p1, boolean hasAnswer, int p2) {
		...
		System.out.print("}");
		System.out.println("]");
		SRM160_Div2_L2_Intersect obj;
		int answer;
		obj = new SRM160_Div2_L2_Intersect();
