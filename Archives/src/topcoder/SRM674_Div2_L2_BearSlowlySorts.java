...
public class SRM674_Div2_L2_BearSlowlySorts
{
	public int minMoves(int[] w)
	{
		...
		for (int i = 0; i < n; i++)
			if (w[i] != w2[i]) ordered = false;
		
		int min = w2[0], max = w2[n-1], cnt = 0;
		if (ordered) cnt = 0;
		else if (w[0] == min || w[n-1] == max) cnt = 1;
		else if (w[0] == max && w[n-1] == min) cnt = 3;
		else cnt = 2;
	...
	private static boolean KawigiEdit_RunTest(int testNum, int[] p0, boolean hasAnswer, int p1) {
		...
		System.out.print("}");
		System.out.println("]");
		SRM674_Div2_L2_BearSlowlySorts obj;
		int answer;
		obj = new SRM674_Div2_L2_BearSlowlySorts();
