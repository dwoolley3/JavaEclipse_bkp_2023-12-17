...
public class grid_SRM590_Div2_L1_FoxAndGomoku
{
    int n, dx, dy;
	public String win(String[] board)
	{
        //Check all 8 directions for 5 o's
        n = board.length;
        for (int i = 0; i < n; i++)
            for (int j = 0; j < n; j++)
                for (dx = -1; dx <= 1; dx++)
                    for (dy = -1; dy <= 1; dy++)
                    {
                        ...
                        if (check(board, i,j)) return "found";
                    }           
        return "not found";
	}

    private boolean check(String[] b, int x, int y)
    {
        int nx, ny;
        for (int i = 0; i < 5; i++)
        {
            nx = x + dx*i;
            ny = y + dy*i;
            if (nx < 0 || nx >= n || ny < 0 || ny >= n || b[nx].charAt(ny) != 'o')
                return false;
        }
        return true;
    }
	...
	private static boolean KawigiEdit_RunTest(int testNum, String[] p0, boolean hasAnswer, String p1) {
		...
		System.out.print("}");
		System.out.println("]");
		grid_SRM590_Div2_L1_FoxAndGomoku obj;
		String answer;
		obj = new grid_SRM590_Div2_L1_FoxAndGomoku();
