...
public class SRM813_Div2_L2_LooRollPyramid_alt3
{
	public int[] countMissing(int Q, int[] A, long[] B)
	{
		int[] ans = new int[Q];
		for (int i = 0; i < Q; i++) {
			long s = 1L*A[i]*(A[i]+1)/2;
			long cur = 0;
			for (int j = 30; j >= 0; j--) {
				if (cur+(1<<j) <= A[i]) {
					long p = (cur+(1<<j)) * (cur+(1<<j) + 1) / 2;
					if (p + B[i] <= s) cur += 1<<j;
				}
			}
			long lef = s - B[i] - cur * (cur+1) / 2;
			ans[i] = (int)lef;
		}
		return ans;
	}
	...
	private static boolean KawigiEdit_RunTest(int testNum, int p0, int[] p1, long[] p2, boolean hasAnswer, int[] p3) {
		...
		System.out.print("}");
		System.out.println("]");
		SRM813_Div2_L2_LooRollPyramid_alt3 obj;
		int[] answer;
		obj = new SRM813_Div2_L2_LooRollPyramid_alt3();
