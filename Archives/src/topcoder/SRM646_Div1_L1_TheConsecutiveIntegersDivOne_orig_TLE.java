...
public class SRM646_Div1_L1_TheConsecutiveIntegersDivOne_orig_TLE
{
	public int find(int[] num, int k)
	{
		...
		{				
			for (int b = num[i]; b <= num[i+k-1] - (k-1); b++)
			{
				int sum2 = 0;			
				for (int j = 0; j < k; j++)
					sum2 += Math.abs(num[i+j] - (b+j)); 
	...
	private static boolean KawigiEdit_RunTest(int testNum, int[] p0, int p1, boolean hasAnswer, int p2) {
		...
		System.out.print("}" + "," + p1);
		System.out.println("]");
		SRM646_Div1_L1_TheConsecutiveIntegersDivOne_orig_TLE obj;
		int answer;
		obj = new SRM646_Div1_L1_TheConsecutiveIntegersDivOne_orig_TLE();
