...
public class SRM606_Div2_L2_EllysNumberGuessing
{
	public int getNumber(int[] guesses, int[] answers)
	{
		...
		for (int j = 0; j <= 1; j++)
		{
			if (g[j] >= 1 && g[j] <= 1_000_000_000)
			{
				good[j] = true;			
				for (int i = 1; i < n; i++)
					if (Math.abs(g[j] - guesses[i]) != answers[i])
						good[j] = false;
			}
		}
		
		if (good[0] && good[1]) return -1;
		if (good[0]) return g[0];
		if (good[1]) return g[1];
		return -2;
	}
	...
	private static boolean KawigiEdit_RunTest(int testNum, int[] p0, int[] p1, boolean hasAnswer, int p2) {
		...
		System.out.print("}");
		System.out.println("]");
		SRM606_Div2_L2_EllysNumberGuessing obj;
		int answer;
		obj = new SRM606_Div2_L2_EllysNumberGuessing();
