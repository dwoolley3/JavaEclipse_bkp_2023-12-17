...
public class SRM680_Div1_L1_BearFair2_WA
{
	public String isFair(int n, int b, int[] upTo, int[] qty)
	{
		int q = upTo.length;
		for (int i = 0; i < q; i++)
			for (int j = i+1; j < q; j++)
				if (upTo[i] > upTo[j])
				{
					int t = upTo[i]; upTo[i] = upTo[j]; upTo[j] = t;
					t = qty[i]; qty[i] = qty[j]; qty[j] = t;
				}
		...
		int mustBeOdd = 0, mustBeEven = 0;
		
		for (int i = 1; i < q+2; i++)
		{
			int uDif = u2[i]-u2[i-1];
			int qDif = q2[i]-q2[i-1];
			if (uDif < qDif || q2[i-1]>q2[i])
				ans = UNFAIR;
			else
			{
				int maxOdd = 0, maxEven = 0;
				for (int j = u2[i-1]; j <= u2[i]; j++)
					if (j%2==1) maxOdd++; maxEven++;
					
				if (uDif - maxOdd > 0)
					mustBeEven += Math.min(qDif - maxOdd, maxEven);
				if (uDif - maxEven > 0)
					mustBeOdd += Math.min(qDif - maxEven, maxOdd);
			}
		}
		
		if (mustBeOdd > n/2 || mustBeEven > n/2)
			ans = UNFAIR;
	...
	private static boolean KawigiEdit_RunTest(int testNum, int p0, int p1, int[] p2, int[] p3, boolean hasAnswer, String p4) {
		...
		System.out.print("}");
		System.out.println("]");
		SRM680_Div1_L1_BearFair2_WA obj;
		String answer;
		obj = new SRM680_Div1_L1_BearFair2_WA();
