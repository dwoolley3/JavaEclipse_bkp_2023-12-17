...
public class TCO2019_R1A_L3_EllysTicketPrices_orig
{
	public double getPrice(int N, int[] C, int target)
	{
		//Find the highest value for which check() is ok
		double eps = 0.005;
		double lo = 0, hi = 1_000_000, mid = 0;
		while (lo < hi)
		{
			mid = lo + (hi - lo) / 2;
			if (check(mid, N, C) < target)
				lo = mid;
			else
				hi = mid;
			if (Math.abs(check(mid, N, C) - target) < eps) break;
		}
		
		System.out.println(mid);
		double ans = ((int)((mid*100) + 0.5)) / 100.0;
		return ans;
	}
	
	private double check(double mid, int n, int[] c)
	{
		double p = mid;
		double sum = p;
		for (int i = 0; i < n-1; i++)
	...
	private static boolean KawigiEdit_RunTest(int testNum, int p0, int[] p1, int p2, boolean hasAnswer, double p3) {
		...
		System.out.print("}" + "," + p2);
		System.out.println("]");
		TCO2019_R1A_L3_EllysTicketPrices_orig obj;
		double answer;
		obj = new TCO2019_R1A_L3_EllysTicketPrices_orig();
