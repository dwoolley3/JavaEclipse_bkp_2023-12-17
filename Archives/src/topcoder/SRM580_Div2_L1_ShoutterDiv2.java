...
public class SRM580_Div2_L1_ShoutterDiv2
{
	public int count(int[] s, int[] t)
	{
		...
		for (int i = 0; i < n; i++)
			for (int j = i+1; j < n; j++)
				...
				//if (Math.min(t[i],t[j]) >= Math.max(s[i], s[j]))
				if (s[i] <= t[j] && s[j] <= t[i])
					cnt++;
		return cnt;				
	}
	...
	{
		if (s1 >= s2 && s1 <= t2) return true;
		if (t1 >= s2 && t1 <= t2) return true;
		if (s2 >= s1 && s2 <= t1) return true;
		if (t2 >= s1 && t2 <= t1) return true;
		return false;
	}
	...
	private static boolean KawigiEdit_RunTest(int testNum, int[] p0, int[] p1, boolean hasAnswer, int p2) {
		...
		System.out.print("}");
		System.out.println("]");
		SRM580_Div2_L1_ShoutterDiv2 obj;
		int answer;
		obj = new SRM580_Div2_L1_ShoutterDiv2();
