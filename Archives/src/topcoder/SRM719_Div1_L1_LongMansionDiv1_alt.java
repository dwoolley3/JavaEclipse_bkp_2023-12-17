...
public class SRM719_Div1_L1_LongMansionDiv1_alt
{
	public long minimalTime(int[] t, int sX, int sY, int eX, int eY)
	{
		...
		for (int x = 0; x < n; x++) {
			long sum = 0;
			for (int i = Math.min(x, sX); i <= Math.max(x, sX); i++) 
				sum += t[i];
			
			int cols = Math.abs(sY - eY) - 1;
			sum += 1L * cols * t[x];
			
			for (int i = Math.min(x, eX); i <= Math.max(x, eX); i++) 
				sum += t[i];
	...
	private static boolean KawigiEdit_RunTest(int testNum, int[] p0, int p1, int p2, int p3, int p4, boolean hasAnswer, long p5) {
		...
		System.out.println("]");
		SRM719_Div1_L1_LongMansionDiv1_alt obj;
		long answer;
		obj = new SRM719_Div1_L1_LongMansionDiv1_alt();
