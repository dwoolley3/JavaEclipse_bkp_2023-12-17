...
public class SRM545_Div2_L2_dp_PillarsDivTwo
{
	public double maximalLength(int[] height, int w)
	{
        ...
        for (int i = 1; i < n; i++)
        {
            dp[i][0] = Math.max(
                dp[i-1][0] + w,
                dp[i-1][1] + hypot(w, height[i-1]-1));
            dp[i][1] = Math.max(
                dp[i-1][1] + hypot(w, height[i] - height[i-1]),
                dp[i-1][0] + hypot(w, height[i]-1));
        }
	...
	private static boolean KawigiEdit_RunTest(int testNum, int[] p0, int p1, boolean hasAnswer, double p2) {
		...
		System.out.print("}" + "," + p1);
		System.out.println("]");
		SRM545_Div2_L2_dp_PillarsDivTwo obj;
		double answer;
		obj = new SRM545_Div2_L2_dp_PillarsDivTwo();
