      ...
      try {
        ...
        if (proc != null) {
            ...
            if (lightsRet.length != N) {
                addFatalError("Your return must contain " + N + " elements, but it contained " + lightsRet.length + ".");
                return 0;
            }
            ...
            for (int i = 0; i < N; ++i) {
                if (lightsRet[i] < 0 || lightsRet[i] >= N) {
                    addFatalError("All elements of your return must be between 0 and " + (N-1) + ", and your return contained " + lightsRet[i] + ".");
                    return 0;
                }
                if (usedP[lightsRet[i]]) {
                    addFatalError("All elements of your return must be unique, and your return contained " + lightsRet[i] + " twice.");
                    return 0;
                }
...
// ------------- visualization part ----------------------
        ...
        public void paint(Graphics g) {
          try {
            ...
            for (int c = 0; c < W; ++c) {
                ...
                for (int k = 0; k < 2; ++k) {
                    g2.drawLine(c * SZ + SZ/2, r * SZ + SZ/2, c * SZ + SZ/2 + dc[typeDirs[type][k]] * (SZ/2-d), r * SZ + SZ/2 + dr[typeDirs[type][k]] * (SZ/2-d));
                }
            }
    ...
    // ---------------------------------------------------
    public GarlandOfLightsVis(String seed) {
    ...
    public static void main(String[] args) {
        ...
        if ((seed.equals("1") || seed.equals("2")) && SZ < 30)
            SZ = 30;
        if (chains)
            debug = true;
        if (save) {
        ...
        vis = true;
        GarlandOfLightsVis f = new GarlandOfLightsVis(seed);
    }
    // ---------------------------------------------------
