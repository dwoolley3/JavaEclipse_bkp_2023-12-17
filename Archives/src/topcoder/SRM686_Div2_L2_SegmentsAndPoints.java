...
public class SRM686_Div2_L2_SegmentsAndPoints
{
	public String isPossible(int[] p, int[] l, int[] r)
	{
		...
		for (int i = 0; i < n; i++)
			for (int j = i+1; j < n; j++)
				if (r[i] > r[j] || (r[i] == r[j] && l[i] > l[j]))
				{
					int x = r[i]; r[i] = r[j]; r[j] = x;
					x = l[i]; l[i] = l[j]; l[j] = x;
				}
	...
	private static boolean KawigiEdit_RunTest(int testNum, int[] p0, int[] p1, int[] p2, boolean hasAnswer, String p3) {
		...
		System.out.print("}");
		System.out.println("]");
		SRM686_Div2_L2_SegmentsAndPoints obj;
		String answer;
		obj = new SRM686_Div2_L2_SegmentsAndPoints();
